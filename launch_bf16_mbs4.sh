#!/bin/bash
#
# launch_bf16_mbs4.sh — Experiment: isolate MBS effect at BF16 precision.
#
# Sibling of launch.sh. Purpose: fill the 4th cell of the {BF16, FP8} x {MBS=2, MBS=4}
# ablation table. With this run we can decompose the stacked gain into
# "how much came from FP8" vs "how much came from MBS".
#
# Differences vs launch.sh:
#   - 8B MBS:        2 -> 4               (more arithmetic intensity per HBM load)
#   - Selective recompute: added         (frees activation memory for MBS=4)
#   - PROJECT_NAME:  gipfelsturm-stacked (same project as stacked runs)
#
# Deliberately NOT changed:
#   - Precision: stays BF16 (no FP8) so the MBS effect is isolated
#   - Embeddings: stays untied (so it doesn't also gain from tying)
#   - Dataloader: still creates the mask (so no extra free win)
#
# Usage: ./launch_bf16_mbs4.sh throughput 8b 50 1
...
# Logging
PROJECT_NAME=gipfelsturm-stacked
