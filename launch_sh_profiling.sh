#!/bin/bash
#
# Usage: ./launch.sh <mode> <model_size> [steps|profiler] [nodes]
#
# Modes:     throughput  (50 steps, no W&B logging)
#            train       (N steps, with W&B and Tensorboard)
#            profile     (50 steps, nsys profiling steps 10-12, W&B enabled, debug partition, default 1 node)
#
# Sizes:     125m, 350m, 760m, 1.5b, 3b, 8b
#
# Profiler:  nsys (default) — only applies to profile mode
#            nsys: wraps rank 0 with nsys, captures cudaProfilerApi range (steps 10-12)
#
# Steps:     required for train mode (e.g., 1000, 5000, 15000)
# Nodes:     optional, default 4 for throughput/train, 1 for profile (max 8)
#
...
case $MODE in
    ...
    --log-timers-to-tensorboard" # tensorboard-dir so we have a Writer so training_log() logs loss, etc..; timers-to-tensorboard for tokens-per-sec logging from the patch
        WANDB=true
        ;;
    train)
...
if [ -n "$WANDB_API_KEY" ]; then
    ...
    TRAINING_CMD="$TRAINING_CMD \
        ...
        --wandb-entity gipfelsturm-lsaie-ss26 \
...
#SBATCH --account=lsaie-ss26 # CHANGED TO OWN ACCOUNT
#SBATCH --time=${TIME}
...
[ -n "$PARTITION" ] && echo "#SBATCH --partition=$PARTITION" >> "$SCRIPT"
...
################ Configs ################
WORKDIR=/users/course_00338/gipfelsturm # CHANGED TO OWN WORKING DIR 
...
if [ "$MODE" = "profile" ] && [ "$PROFILER" = "nsys" ]; then
    cat >> "$SCRIPT" << 'PROFILE_FOOTER'
...
export TRAINING_CMD LOG_DIR
srun -lu --mpi=pmix --network=disable_rdzv_get --environment=alps3 --cpus-per-task $SLURM_CPUS_PER_TASK --wait 60 bash -c 'if [ $SLURM_PROCID -eq 0 ]; then nsys profile --capture-range=cudaProfilerApi --capture-range-end=stop --trace=cuda,nvtx,nccl -o $LOG_DIR/nsys_profile_rank0_$SLURM_JOB_ID numactl --membind=0-3 $TRAINING_CMD; else numactl --membind=0-3 $TRAINING_CMD; fi'

echo "Profile output: $LOG_DIR/nsys_profile_rank0_$SLURM_JOB_ID.nsys-rep"
echo "END TIME: $(date)"
PROFILE_FOOTER
else
    cat >> "$SCRIPT" << 'FOOTER'
