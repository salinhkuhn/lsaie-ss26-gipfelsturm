#!/bin/bash
#
# launch_stacked_3b.sh — Experiment: stacked throughput improvements at 3B scale.
#
# Sibling of launch.sh. Same stack as launch_stacked.sh but for a smaller model,
# -> higher tok/s/GPU because per-step fixed costs (kernel
# launches, comm, dataloader) amortize over fewer FLOPs.
#
# Differences vs launch.sh:
#   - 3B MBS:        4 -> 8               (smaller model -> bigger batch fits)
#   - FP8 hybrid:    added                (matmul throughput ~2x vs BF16 on H200)
#   - Selective recompute: added         (frees activation memory for bigger MBS)
#   - Tied embeddings: untie -> tie      (smaller params, less DP traffic)
#   - No-mask in dataloader: added       (FlashAttention builds mask implicitly)
#   - PROJECT_NAME:  gipfelsturm-stacked 
#
# Usage: ./launch_stacked_3b.sh throughput 3b 50 1
